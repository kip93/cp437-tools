    ...
    if meta.title().is_some() {
        output.write(
            format!(
                ...
                if meta::check_title(Some(meta)).is_err() {
                    "1;3;31"
                } else {
                    "3;32"
                },
    ...
    if meta.author().is_some() {
        output.write(
            format!(
                ...
                if meta::check_author(Some(meta)).is_err() {
                    "1;3;31"
                } else {
                    "3;32"
                },
    ...
    if meta.group().is_some() {
        output.write(
            format!(
                ...
                if meta::check_group(Some(meta)).is_err() {
                    "1;3;31"
                } else {
                    "3;32"
                },
    ...
    if meta.date().is_some() {
        output.write(
            format!(
                ...
                if meta::check_date(Some(meta)).is_err() {
                    "1;3;31"
                } else {
                    "3;32"
                },
    ...
    output.write(
        format!(
            ...
            match meta.r#type {
                (0, _) => format!(
                    "\x1B[1;3;33mNone ({})",
                    meta::type_name(Meta::default().r#type)
                ),
                (1, 0) | (1, 1) => format!("\x1B[3;32m{}", meta::type_name(meta.r#type)),
...
fn print_font(output: &mut Output, meta: &Meta) -> ExitCode {
    output.write(
        format!(
            ...
            if meta.font().is_none() {
                format!("\x1B[1;3;33m<N/A> ({})", Meta::default().font)
            } else if ["IBM VGA", "IBM VGA 437"].contains(&meta.font().unwrap().as_str()) {
                format!("\x1B[3;32m{:?}", meta.font().unwrap())
            } else {
                format!("\x1B[1;3;31m{:?}", meta.font().unwrap())
            }
        )
        .as_bytes(),
    )?;
    ...
    for (i, note) in meta.notes().iter().enumerate() {
        output.write(
            format!(
                ...
                i,
                if meta::check_note(Some(meta), i).is_err() {
                    "1;3;31"
                } else {
                    "3;32"
                },
...
mod tests {
    ...
    fn no_input() {
        assert_eq!(
            run(vec![String::from("cp437-read-meta")]),
