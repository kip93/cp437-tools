...
impl Input {
    pub fn new(input: &String) -> Result<Self, ExitCode> {
        let mut real_input = File::open(input)?;

        let meta = meta::read(&mut real_input)?;

        let size = match meta {
            Some(ref meta) => meta.size,
            None => real_input.metadata()?.len() as u32,
        };
    ...
    pub fn read_by_chunks<'a, F: for<'b> FnMut(&'b [u8]) -> Result<(), ExitCode> + 'a>(
        ...
        self.real_input.rewind()?;

        let mut chunk = vec![0; 1 << 12]; // 4k chunks
        let mut reader = BufReader::with_capacity(chunk.len(), &self.real_input);

        let mut index = 0;
        while index < self.size {
            let count = reader.read(&mut chunk)? as u32;
    ...
    #[inline]
    pub fn read_by_bytes_full<
        'a,
        F: for<'b> FnMut(u8, (u16, u16), [[u8; 3]; 2]) -> Result<(), ExitCode> + 'a,
    >(
        &mut self,
        mut callback: F,
        scheme: &String,
        ...
        return self.read_by_bytes(|byte| {
            ...
            if !control.is_empty() {
                if byte == b'm' {
                    ...
                    x = min(
                        x + String::from_utf8(control[2..].to_vec())?.parse::<u16>()?,
                        meta.width() - 1,
                    );
                    control.clear();
...
pub struct Output {
    real_output: Box<dyn Write>,
}

impl Output {
    pub fn file(output: &String) -> Result<Self, ExitCode> {
        let real_output = Box::new(File::create_new(output)?) as Box<dyn Write>;
        return Ok(Self { real_output });
    }

    pub fn stdout() -> Result<Self, ExitCode> {
        let real_output = Box::new(stdout()) as Box<dyn Write>;
        return Ok(Self { real_output });
    }
...
impl Write for Output {
    fn write(&mut self, buf: &[u8]) -> Result<usize, io::Error> {
        return self.real_output.write(buf);
    }

    fn flush(&mut self) -> Result<(), io::Error> {
        return self.real_output.flush();
    }
}

#[inline]
pub fn process<F: for<'a> FnOnce(&'a mut Input, &'a mut Output) -> ExitCode>(
    input: &String,
    callback: F,
) -> ExitCode {
